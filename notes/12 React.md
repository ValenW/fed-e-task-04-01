...
<img src="./12 React.assets/1/5.gif" />
...
<img src="./12 React.assets/1/6.png" width="70%"/>
...
<img src="./12 React.assets/2/3.png" width="50%"/>
...
<img src="./12 React.assets/2/8.png" width="80%" style="margin-bottom: 30px"/>
...
<img src="./12 React.assets/2/5.png" width="40%" align="left"/>
...
<img src="./12 React.assets/4/3.png" width="40%"/>

<img src="./12 React.assets/4/4.png" width="40%"/>
...
<img src="./12 React.assets/4/7.png" width="90%" align="left"/>
...
2. 创建FiberRoot和RootFiber, 并将其互相连接

   `FiberRoot.current = RootFiber, RootFiber.stateNode = FiberRoot`

3. 向container挂载`_reactRootContainer`属性: `{ _internalRoot: FiberRoot }`
...
<img src="./12 React.assets/4/5.png" width="80%" align="left"/>
...
export function getPublicRootInstance(
  ...
  switch (containerFiber.child.tag) {
    // 普通
    case HostComponent:
      // getPublicInstance直接返回参数, 即containerFiber.child.stateNode
...
export function updateContainer(
  ...
  // 初始化渲染时, 任务同步执行不涉及被打断的问题, 过期时间被设置为常量 1073741823
...
进入render阶段, 构建WIP Fiber树, 主要3步:

1. prepareFreshStack: 构建WIP tree和rootFiber
2. workLoopSync: 构建其他Fiber对象
3. finishSyncRender: 结束render, 进入commit阶段
...
构建root fiber对象, 即div#root所对应的fiber对象
...
function prepareFreshStack(root, expirationTime) {
  ...
  // 建构 workInProgress Fiber 树的 root 对象
...
1. beginWork: 自上而下构建fiber对象
2. completeUnitOfWork: 自下而上构建fiber对象
...
根据WIP的tag类型调用不同的渲染方法

构建时tag顺序为3, 2, 5: root, component, function
...
/****************************** 构建子节点fiber对象并挂载到workInProgress.child **************************/
    // 客户端渲染走 else
    // 构建子节点 fiber 对象
    reconcileChildren(
      current,
...
根据子节点type不同调用不同方法构建子节点的fiber对象
 ...
 * 1. 创建其他节点的 Fiber 对象, 如兄弟节点的child: 返回优先: child > sibling > return
 * 2. 创建每一个节点的真实 DOM 对象并将其添加到 stateNode 属性中: completeWork
 * 3. 构建 effect 链表结构
 */
...
创建节点对应DOM对象, 并创建所有子节点对应的DOM对象, 然后append到父节点上
...
commitBeforeMutationEffectOnFiber: 为类组件调用 getSnapshotBeforeUpdate 生命周期函数
...
1. 类组件的生命周期函数: componentDidUpdate, componentDidMount
2. 函数数组件的钩子函数
